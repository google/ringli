...
int ReadHuffmanCodeLengths(const uint8_t* code_length_code_lengths,
  ...
  while (symbol < num_symbols && space > 0) {
    ...
    RingliBitReaderFillWindow(br, 5);
    p += (br->val >> br->bit_pos) & 31;
    br->bit_pos += p->bits;
    ...
    if (code_len < kCodeLengthRepeatCode) {
      ...
      repeat += (int)RingliBitReaderReadBits(br, extra_bits) + 3;
...
bool HuffmanDecodingData::ReadFromBitStream(int alphabet_size,
  ...
  simple_code_or_skip = (int)RingliBitReaderReadBits(br, 2);
  if (simple_code_or_skip == 1) {
    ...
    int max_bits = 0;
    int symbols[4] = {0};
    const int num_symbols = (int)RingliBitReaderReadBits(br, 2) + 1;
    while (max_bits_counter) {
      max_bits_counter >>= 1;
      ++max_bits;
    }
    for (i = 0; i < num_symbols; ++i) {
      symbols[i] = (int)RingliBitReaderReadBits(br, max_bits) % alphabet_size;
    ...
    for (i = simple_code_or_skip; i < kCodeLengthCodes && space > 0; ++i) {
      ...
      uint8_t v;
      RingliBitReaderFillWindow(br, 4);
      p += (br->val >> br->bit_pos) & 15;
      br->bit_pos += p->bits;
      v = (uint8_t)p->value;
