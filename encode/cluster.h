...
#ifndef ENCODE_CLUSTER_H_
#define ENCODE_CLUSTER_H_
...
inline bool operator<(const HistogramPair& p1, const HistogramPair& p2) {
  ...
  DCHECK(p1.idx1 < p1.idx2);
  DCHECK(p2.idx1 < p2.idx2);
...
template <typename HistogramType>
int HistogramCombine(HistogramType* out, int* cluster_size, uint32_t* symbols,
                     int symbols_size, int max_clusters) {
  ...
  // reduction. For efficiency, only the front of the queue matters, the rest
  // of it is unordered.
  std::vector<HistogramPair> pairs;
...
double HistogramBitCostDistance(const HistogramType& histogram,
  ...
  return PopulationCost(tmp) - candidate.bit_cost;
}
...
void ClusterHistograms(const std::vector<HistogramType>& in, int num_contexts,
  ...
  for (int i = 0; i < in_size; ++i) {
    (*out)[i] = in[i];
    (*out)[i].bit_cost = PopulationCost(in[i]);
  ...
  if (block_group_offsets.size() > 1) {
    ...
    for (int i = 0; i < block_group_offsets.size(); ++i) {
      ...
      int length = ((i + 1 < block_group_offsets.size()
                         ...
                         : in_size) -
                    offset);
      int nclusters = HistogramCombine(&(*out)[0], &cluster_size[0],
  ...
  if (block_group_offsets.size() <= 1 || num_clusters > max_histograms) {
    // If we did not have block groups or the per-block-group clustering
    // ended with too many histograms, we have to do one final round of
    // clustering.
    num_clusters =
