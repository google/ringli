...
// We use only the context map alphabet in ringli, where the maximum alphabet
...
void StoreHuffmanTree(const uint8_t* depths, size_t num, size_t* storage_ix,
                      uint8_t* storage) {
  // Write the Huffman tree into the compact representation.
  DCHECK(num <= kMaxAlphabetSize);
  ...
  // Calculate the statistics of the Huffman tree in the compact
  // representation.
  ...
  StoreHuffmanTreeToBitMask(huffman_tree_size, huffman_tree,
                            ...
                            code_length_bitdepth_symbols, storage_ix, storage);
}
