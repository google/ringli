...
bool RebalanceHistogram(const float* targets, int max_symbol, int table_size,
  ...
  for (int n = 0; n < max_symbol; ++n) {
    if (targets[n] > 0) {
      ...
      // SmallestIncrement (when minimize_error_of_sum is false) or one of
      // two closest so as to keep the sum as close as possible to
      // sum_nonrounded.
...
void NormalizeCounts(int* counts, int* omit_pos, const int length,
  ...
  *num_symbols = symbol_count;
  ...
  DCHECK_LE(symbol_count, table_size);

  const float norm = 1.f * table_size / total;
  float targets[MAX_SYMBOLS];
  for (int n = 0; n < max_symbol; ++n) {
  ...
  if (!RebalanceHistogram<false>(targets, max_symbol, table_size, omit_pos,
    ...
    CHECK(RebalanceHistogram<true>(targets, max_symbol, table_size, omit_pos,
                                   counts));
  }
}
...
void EncodeCounts(const int* counts, const int omit_pos, const int num_symbols,
  ...
  if (num_symbols <= 2) {
    ...
    int length = 0;
    int logcounts[MAX_SYMBOLS] = {0};
    int omit_log = 0;
    for (int i = 0; i < MAX_SYMBOLS; ++i) {
      DCHECK_LE(counts[i], ANS_TAB_SIZE);
      DCHECK_GE(counts[i], 0);
      if (i == omit_pos) {
        length = i + 1;
      } else if (counts[i] > 0) {
    ...
    // length - 3.
    WriteBits(max_bits, length - 3, storage_ix, storage);
    ...
    for (int i = 0; i < length; ++i) {
      if (logcounts[i] > 1 && i != omit_pos) {
        ...
        DCHECK_EQ(counts[i] & ((1 << drop_bits) - 1), 0);
